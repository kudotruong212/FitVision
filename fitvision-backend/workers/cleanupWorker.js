...
// Không khởi tạo queue / cron jobs trong môi trường test để tránh giữ open handles
if (process.env.NODE_ENV !== 'test' && config.redis.url) {
  cleanupQueue = new Queue('cleanup', config.redis.url);
  ...
  // Schedule recurring cleanup jobs (production / non-test only)
  cleanupQueue.add(
    'cleanup-old-scans',
    { daysOld: 30 },
    {
